/* =================================================================== */
/* APP.JS V16.0 - CONTROLE CENTRAL (STRAVA LOOP + FINANCEIRO ROUTE)
/* =================================================================== */
...
const AppPrincipal = {
    ...
    initPlatform: function() {
        ...
        document.getElementById('nav-planilha-btn').onclick = function() { AppPrincipal.navigateTo('planilha'); };
        document.getElementById('nav-feed-btn').onclick = function() { AppPrincipal.navigateTo('feed'); };
        
        const finBtn = document.getElementById('nav-finance-btn');
        if (finBtn) finBtn.onclick = function() { AppPrincipal.navigateTo('finance'); };
        ...
        document.querySelectorAll('.close-btn').forEach(function(b) {
            b.onclick = function(e) { e.target.closest('.modal-overlay').classList.add('hidden'); };
        });
        ...
        // Bind do Financeiro (Garante que o listener exista se o form já estiver no DOM)
        ...
        AppPrincipal.state.auth.onAuthStateChanged(function(user) {
    ...
    loadUserData: function(uid) {
        AppPrincipal.state.db.ref('users').on('value', function(s) { AppPrincipal.state.userCache = s.val() || {}; });
        
        AppPrincipal.state.db.ref('users/' + uid).once('value', function(s) {
            let data = s.val();
            AppPrincipal.state.db.ref('admins/' + uid).once('value', function(adminSnap) {
                ...
                if (data) {
                    ...
                    AppPrincipal.state.db.ref(`users/${uid}/stravaAuth`).on('value', function(ts) { AppPrincipal.state.stravaTokenData = ts.val(); });
    ...
    updateClasses: function(isAdmin) {
    ...
    handleLogout: function() { AppPrincipal.state.auth.signOut().then(function() { window.location.href = 'index.html'; }); },

    // --- STRAVA DEEP SYNC (LOOP COMPLETO) ---
    handleStravaConnect: function() { window.location.href = `https://www.strava.com/oauth/authorize?client_id=${window.STRAVA_PUBLIC_CONFIG.clientID}&response_type=code&redirect_uri=${window.STRAVA_PUBLIC_CONFIG.redirectURI}&approval_prompt=force&scope=read_all,activity:read_all,profile:read_all`; },
    
    exchangeStravaCode: async function(code) {
        try {
    ...
    handleStravaSyncActivities: async function() {
    ...
    // MODAL FEEDBACK
    openFeedbackModal: function(workoutId, ownerId, title) {
        ...
        AppPrincipal.state.db.ref(`data/${ownerId}/workouts/${workoutId}`).once('value', function(s) {
            if (s.exists()) {
                const d = s.val();
                ...
                if (d.stravaData) {
                    ...
                    if (d.stravaData.splits) {
                        ...
                        d.stravaData.splits.forEach(function(sp) {
        ...
        AppPrincipal.state.db.ref(`workoutComments/${workoutId}`).on('value', function(s) {
            const list = document.getElementById('comments-list');
            if (list) {
                list.innerHTML = "";
                if (!s.exists()) return;
                s.forEach(function(c) { list.innerHTML += `<div class="comment-item"><b>${c.val().name}:</b> ${c.val().text}</div>`; });
            }
        });
    ...
    handleLogActivitySubmit: async function(e) { 
        e.preventDefault();
    ...
    fileToBase64: function(file) { return new Promise((r, j) => { const reader = new FileReader(); reader.onload = () => r(reader.result.split(',')[1]); reader.onerror = j; reader.readAsDataURL(file); }); },
    
    // Upload de Foto de Perfil
    handleProfilePhotoUpload: async function(e) {
        const file = e.target.files[0];
        if (!file) return;
        try {
    ...
    callGeminiTextAPI: async function(prompt) {
    ...
    handleSaveIaAnalysis: async function() {
    ...
    uploadFileToCloudinary: async function(file, folder) {
